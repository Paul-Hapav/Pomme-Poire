...
void captureImg(uint16_t width, uint16_t height, bool chroma){
  ...
  // for (y = 0; y < height; y++) {
  //   for (x = 0; x < width; x++) {
  //     byte intensity = frame[y][x] & 0xF0; // Ajuste l'intensit̩ pour extraire la valeur
  //     uint16_t color = tft.color565(intensity, intensity, intensity); // Convertit en couleur 16 bits
  //     tft.writePixel(x, y, color); // Utilise writePixel pour un affichage plus rapide
  //   }
  // }
  // tft.endWrite();
}
